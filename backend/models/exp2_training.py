"""
Experiment 2 Training Script
─────────────────────────────
Pre-LayerNorm Vision Transformer with RESIDUAL connections.
Architecture: patch embed → [LN → MHA → residual, LN → FFN → residual] × L → LN → cls head

Computes:
  ✦ Training loss, validation loss
  ✦ Training accuracy, validation accuracy
  ✦ Stable rank per layer (SVD-based), stable rank per step
"""
...
# ── Attention Head (returns attn weights for SR) ─────────────
...
# ── Pre-LN Transformer Block (WITH residual + LayerNorm) ────

class PreLNBlock(nn.Module):
    """Pre-LayerNorm block WITH residual connections."""
    def __init__(self, embed_dim, num_heads):
        super().__init__()
    ...
    def forward(self, x):
        sa_out, attn = self.sa(self.ln1(x))
...
class ResidualLNViT(nn.Module):
    def __init__(self, num_layers=6, num_heads=4, embed_dim=128,
        ...
        self.blocks = nn.ModuleList([
            PreLNBlock(embed_dim, num_heads) for _ in range(num_layers)
        ])
