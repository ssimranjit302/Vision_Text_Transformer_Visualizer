...
class ResidualTransformerBlock(nn.Module):
    def __init__(self, embed_dim: int, num_heads: int):
        ...
        self.attn = ResidualMultiHeadAttention(embed_dim, num_heads)
        self.ln2  = nn.LayerNorm(embed_dim)
        self.ffn  = ResidualFeedForward(embed_dim)
...
class ResidualTextTransformer(nn.Module):
    ...
    Residual Transformer for text classification.
      - Pre-LayerNorm  (ln1, ln2 in block, ln_f at end)
      - Residuals  (x = x + f(x))
    ...
    def __init__(self, vocab_size: int, embed_dim: int = 128, num_heads: int = 4,
        ...
        self.blocks      = nn.ModuleList([
            ResidualTransformerBlock(embed_dim, num_heads)
...
def train_epoch(model: ResidualTextTransformer,
...
def evaluate(model: ResidualTextTransformer,
